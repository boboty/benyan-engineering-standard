...
5. 检查实际结果、接口/UI、日志、Secret 与 Git diff，交由 Independent Verifier 独立验收；AI self-check 或 Developer 内部 reviewer 不构成独立验收。PASS 由 Verifier 写入 `PROGRESS.md` 并创建最终任务 commit，Developer 不执行最终任务 commit；采用 AI Task 工作流的项目可从 `templates/PROGRESS.md` 建立该文件。
